...
export function main(ns: NS): void {
	...
	const { hostname, organization, money, security, admin, hackLevel, backdoor, ports } = new ServerInfo(
		ns,
		server,
	)
...
<<>><<>><<>><<>><<>><<>><<>><<>><<>><<>>
Money: ${ns.nFormat(money.available, '$0.000a')} / ${ns.nFormat(money.max, '$0.000a')} (${(
		(money.available / money.max) *
		100
	).toFixed(2)}%)
