	...
	while (growThreads > 0 || weakThreads1 > 0 || weakThreads2 > 0) {
		...
		if (home.calculateThreadCount(1.75) >= growThreads + weakThreads1 + weakThreads2) {
			...
			ns.exec(
				'bin/batch/weaken.js',
				'home',
				Math.min(weakThreads1, home.calculateThreadCount(1.75)),
				target.hostname,
			)
			...
			ns.exec(
				'bin/batch/grow.js',
				'home',
				Math.min(growThreads, home.calculateThreadCount(1.75)),
				target.hostname,
			)
			...
			ns.exec(
				'bin/batch/weaken.js',
				'home',
				Math.min(weakThreads2, home.calculateThreadCount(1.75)),
				target.hostname,
			)
		}
